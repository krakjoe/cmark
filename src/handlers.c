...
zval* php_cmark_node_read(zend_object *object, zend_string *member, int type, void **rtc, zval *rv) {
	php_cmark_node_t *n = php_cmark_node_from(object);
#else
...
#if PHP_VERSION_ID >= 80000
		php_cmark_throw("invalid read of %s", ZSTR_VAL(member));
#else
		php_cmark_throw(
			"invalid read of %s", 
...
#endif
#if PHP_VERSION_ID >= 80000
	php_cmark_throw("invalid write of %s", ZSTR_VAL(member));
#else
	php_cmark_throw(
		"invalid write of %s", 
		Z_TYPE_P(member) == IS_STRING ? 
...
#if PHP_VERSION_ID >= 80000
HashTable* php_cmark_node_debug(zend_object *object, int *tmp) {
#else
...
#if PHP_VERSION_ID >= 80000
	php_cmark_node_debug_impl(debug, php_cmark_node_from(object));
#else
...
#if PHP_VERSION_ID >= 80000
zend_object* php_cmark_node_clone(zend_object *object) {
	php_cmark_node_t *zo = php_cmark_node_from(object);
#else
